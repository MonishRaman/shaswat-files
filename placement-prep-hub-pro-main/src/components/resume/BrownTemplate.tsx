...
interface BrownTemplateProps {
  data: ResumeData;
}

export const BrownTemplate: React.FC<BrownTemplateProps> = ({ data }) => {
  return (
    <div className="flex flex-col md:flex-row min-h-[800px]">
      {/* Left sidebar */}
      <div className="bg-[#5e4534] text-white w-full md:w-64 p-6 flex flex-col">
      ...
      <div className="bg-white flex-1 p-6">
        <h1 className="text-3xl font-bold text-[#5e4534] mb-1">
          {data.personalInfo.fullName}
        </h1>
        <p className="text-gray-600 mb-4">{data.personalInfo.objective}</p>
        
        <hr className="border-[#5e4534]/20 my-4" />
        
        <div className="mb-6">
          <h2 className="text-xl font-bold text-[#5e4534] flex items-center gap-2 mb-3">
